...
class ProfileLoaderTests(unittest.TestCase):
    def test_load_workspace_context_from_discovered_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(memory_root / "WORKSPACE_START_HERE.md", context.workspace_start_here_path)
            self.assertEqual(memory_root / "WORKSPACE_RULES.md", context.workspace_rules_path)
    ...
    def test_profile_loader_fails_closed_when_required_workspace_file_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (memory_root / "WORKSPACE_RULES.md").unlink()

            with self.assertRaisesRegex(ProfileLoadError, "workspace_rules_path"):
